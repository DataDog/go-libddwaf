...
func (e *JsonEncoder) Encode(config object.EncoderConfig, obj *object.WAFObject, _ int) (map[object.TruncationReason][]int, error) {
	defer e.iterCfg.ReturnIterator(e.iter)
	e.config = config
	e.truncations = make(map[libddwaf.TruncationReason][]int)

	err := e.encode(obj, e.config.MaxContainerDepth)
...
func (e *JsonEncoder) encodeString(str string, obj *libddwaf.WAFObject) {
	strLen := len(str)
	if strLen > e.config.MaxStringLength {
		str = str[:e.config.MaxStringLength]
		e.addTruncation(object.StringLength, strLen)
	}
...
func (e *JsonEncoder) encodeMapKeyFromString(keyStr string, obj *libddwaf.WAFObject) {
	size := len(keyStr)
	if size > e.config.MaxStringLength {
		keyStr = keyStr[:e.config.MaxStringLength]
		e.addTruncation(object.StringLength, size)
	}
...
func (e *JsonEncoder) encodeObject(parentObj *libddwaf.WAFObject, depth int) error {
	...
	if depth < 0 {
		e.addTruncation(object.ContainerDepth, e.config.MaxContainerDepth-depth)
		e.iter.Skip()
		if e.iter.Error != nil {
			return e.iter.Error
		}
