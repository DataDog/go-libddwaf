...
func main() {
	...
	for _, tgt := range targets {
		...
		if created || force {
			createVendorEmbedFile(embedDir, tgt)
			createBuildTaggedImport(tgt)
		}
...
// createBuildTaggedImport creates a lib_<os>_<arch>.go file in the libDir, which will be used to select the libddwaf library.
// This is needed because bazel requires the go:embed directive to be in the same package as the directive.
// See bazelbuild/bazel-gazelle#1316 for more details.
func createBuildTaggedImport(tgt target) {
	gosource := strings.Join(
		[]string{
			...
			"// Copyright 2016-present Datadog, Inc.",
			"",
			fmt.Sprintf("//go:build %s && %s && !%s && !datadog.no_waf", tgt.os, tgt.arch, goVersionUnsupported),
			"",
			"package lib",
			"",
			"// THIS FILE IS AUTOGENERATED. DO NOT EDIT.",
			"",
			fmt.Sprintf(`import bin "github.com/DataDog/go-libddwaf/v2/internal/lib/%s-%s"`, tgt.os, tgt.arch),
			"",
...
// createVendorEmbedFile creates a vendor.go file in the embedDir, which will be used to embed the libddwaf library.
// This is needed because bazel requires the go:embed directive to be in the same package as the directive.
// See bazelbuild/bazel-gazelle#1316 for more details.
func createVendorEmbedFile(embedDir string, tgt target) {
	gosource := strings.Join(
		[]string{
			"// Unless explicitly stated otherwise all files in this repository are licensed",
			"// under the Apache License Version 2.0.",
			...
			"",
			fmt.Sprintf("//go:embed libddwaf.%s", tgt.ext),
			"var Libddwaf []byte",
			"", // Trailing new line...
		},
		"\n",
	)
	if err := os.WriteFile(path.Join(embedDir, "vendor.go"), []byte(gosource), 0644); err != nil {
		panic(err)
	}
}
